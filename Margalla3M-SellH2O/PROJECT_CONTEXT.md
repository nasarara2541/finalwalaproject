...
- **Main area (2 columns using CSS grid 1fr 280px):**
  - Left: Stock Receipt Details table — a **global, system-wide ledger** (not
    scoped to one invoice), powered by `get_stock_expiry_panel.php`, sorted
    nearest-expiry-first, merged client-side with whichever invoice is currently
    open/being built (`detailLines`). Only the lines belonging to the invoice
    currently open show a delete (X) button — every other invoice's historical
    rows render read-only, since received stock is never deleted, only edited
    via that invoice's own Modify flow. This matches the reference screenshot,
    where the highlighted row being typed sits inside one big cross-invoice list.
...
For each detail line:
    ...
    ST_STOCKRECEIPTDETAIL.ITEMS_AVAILABLE = ITEMS_RECEIVED   (brand-new batch)
                                          = ITEMS_RECEIVED - already-sold   (Modify, same batch)

ST_STOCKRECEIPT row inserted/updated
ST_STOCKRECEIPTDETAIL rows inserted
```

### Batch-level "Qty Available" and FEFO (matches the professor's reference annotation)

The reference screenshot annotates `Qty Available` as "number of tablets available
on shelf, **decrease with sale**." This is tracked per **batch**
(`ST_STOCKRECEIPTDETAIL.ITEMS_AVAILABLE`), separate from the aggregate
`Item_Stock.QTY_INHAND` that POS search (`search_items.php`) filters on:

- A brand-new detail line starts with `ITEMS_AVAILABLE = ITEMS_RECEIVED` (the
  batch is fully available the moment it's received). Server-side, this is
  computed authoritatively in `save_stock_receipt.php` — the client's own
  `qty_available` value is just for display before saving.
- When a POS sale is saved (`save_transaction.php`), after decrementing the
  aggregate `Item_Stock.QTY_INHAND`, it also walks that stock number's batches
  ordered by `EXPIRY_DATE ASC` (FEFO — first-expiring, first-out) and deducts
  the sold quantity from the earliest-expiring batch(es) first, clamped at 0.
- When a batch's `ITEMS_AVAILABLE` reaches 0 it drops out of
  `get_stock_expiry_panel.php`'s `WHERE ITEMS_AVAILABLE > 0` filter and
  disappears from the ledger — but the row itself is never deleted, since it's
  still a real historical record of stock that was received.
- Modifying an existing invoice preserves this: before the old detail rows are
  deleted, `save_stock_receipt.php` captures each old batch's
  `ITEMS_RECEIVED`/`ITEMS_AVAILABLE`, computes `sold = received - available`,
  and carries that `sold` amount forward onto the new line for the same
  stock+batch — so editing an invoice doesn't wipe out sales that already
  happened against that batch.

### Lock / Modify / New Invoice workflow

Loading a past invoice (from the Invoice List popup) opens it in **view mode**:
header fields, the item-entry row, Save Line, and Save-Stock are all disabled
(`lockForm()`). Clicking **Modify** unlocks them (`unlockForm()`) for editing.
**New Invoice** clears the form and unlocks it for fresh entry. After a
successful **Save-Stock**, the form re-locks (the invoice is now "saved" and
view-only again until Modify is clicked), and the global ledger is refreshed.
...
- Duplicate Stock Number in same invoice is blocked (one batch per item per invoice)
...
1. **`get_stock_by_expiry.php`** — old unused file, was deleted. Replaced by `get_stock_expiry_panel.php`, which is now actually wired up as the Stock Receiving screen's main ledger (see "Lock / Modify / New Invoice workflow" above — it used to be built but never called).
...
8. **POS still allows overselling** — `save_transaction.php` decrements `Item_Stock.QTY_INHAND` without a floor check, so concurrent sales or a manually-typed quantity could in theory push it negative. Not fixed as part of the stock-receiving work; worth a follow-up if it matters for real customer use.

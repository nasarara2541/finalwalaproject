...
//   cancelled_bills                              -> lists voided bills
//   price_changed                                -> Item_Price_History, Items-panel filters
// All revenue-counting reports (total_sale, users_bill_count, day_wise_sale,
// all_sold_items, item_group_summary) exclude cancelled bills -- a voided
// sale isn't real revenue, counting it would silently overstate every
...
$salePerson  = trim($_GET['sale_person'] ?? '');
$description = trim($_GET['description'] ?? '');
$remarks     = trim($_GET['remarks'] ?? '');
$modifyBy    = trim($_GET['modify_by'] ?? '');
...
if ($report === 'total_sale' || $report === 'users_bill_count' || $report === 'day_wise_sale') {
    ...
    if ($description !== '') { $where[] = "t.Description LIKE ?"; $params[] = '%'.$description.'%'; }
    if ($remarks     !== '') { $where[] = "t.Remarks LIKE ?";     $params[] = '%'.$remarks.'%'; }
    if ($modifyBy    !== '') { $where[] = "t.Modified_By LIKE ?"; $params[] = '%'.$modifyBy.'%'; }

    $join = '';
    if ($customer !== '' || $address !== '') {
        // t.Cust_name is NULL on every one of the 1,060 historical bulk-
        // loaded rows even when Customer_id correctly links to a real
        // customer (confirmed live) -- COALESCE falls back to the live
        // Customer master record instead of searching a column that's
        // silently always empty for old data.
        $join .= " LEFT JOIN Customer c ON c.Customer_id = t.Customer_id";
    }
    if ($customer !== '') { $where[] = "COALESCE(t.Cust_name, c.Cust_name) LIKE ?"; $params[] = '%'.$customer.'%'; }
    if ($address  !== '') { $where[] = "c.Address LIKE ?"; $params[] = '%'.$address.'%'; }
    if ($salePerson !== '') {
        $join .= " LEFT JOIN Employee emp ON emp.Emp_no = t.Sale_Person_id";
        $where[] = "emp.Full_Name LIKE ?"; $params[] = '%'.$salePerson.'%';
    }
    ...
    if ($report === 'day_wise_sale') {
        // Sale side and Return side are grouped independently by their own
        // date (a return doesn't have to happen the same day as the
        // original sale) and combined with a FULL OUTER JOIN so a day with
        // only returns (no new sales) still shows up. The Return side only
        // honors the shared date/time range -- the more specific Bills
        // filters (Sale Type, Customer, etc.) don't have a clean meaning
        // against Return_Transaction's own columns, so they're not applied
        // there.
        $whereClause = $where ? (' WHERE ' . implode(' AND ', $where)) : '';

        $returnWhere = []; $returnParams = [];
        addDateRange('Return_date', $from, $to, $returnWhere, $returnParams);
        addTimeRange('Return_date', $timeFrom, $timeTo, $returnWhere, $returnParams);
        $returnWhereClause = $returnWhere ? (' WHERE ' . implode(' AND ', $returnWhere)) : '';

        $sql = "WITH SaleDays AS (
                    SELECT CAST(t.Trans_date AS DATE) AS d, COUNT(*) AS bill_count, ISNULL(SUM(dt.line_total),0) AS total_sale
                    FROM [Transaction] t $detailJoin $join
                    $whereClause
                    GROUP BY CAST(t.Trans_date AS DATE)
                ),
                ReturnDays AS (
                    SELECT CAST(Return_date AS DATE) AS d, ISNULL(SUM(Return_amount),0) AS total_return
                    FROM Return_Transaction
                    $returnWhereClause
                    GROUP BY CAST(Return_date AS DATE)
                )
                SELECT COALESCE(sd.d, rd.d) AS sale_date_raw,
                       CONVERT(VARCHAR(10), COALESCE(sd.d, rd.d), 103) AS sale_date,
                       ISNULL(sd.bill_count,0) AS bill_count,
                       ISNULL(sd.total_sale,0) AS total_sale,
                       ISNULL(rd.total_return,0) AS total_return
                FROM SaleDays sd
                FULL OUTER JOIN ReturnDays rd ON rd.d = sd.d
                ORDER BY sale_date_raw DESC";
        $stmt = runQuery($conn, $sql, array_merge($params, $returnParams));
        $rows = [];
        while ($r = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            $rows[] = [ $r['sale_date'], (int)$r['bill_count'], money($r['total_sale']), money($r['total_return']) ];
        }
        echo json_encode(['title' => 'Day Wise Sale & Return Summary', 'columns' => ['Date','Bills','Total Sale','Total Returns'], 'rows' => $rows]);
        exit;
    }
...
if ($report === 'cancelled_bills') {
    $where = ['t.Is_Cancelled = 1']; $params = [];
    addDateRange('t.Cancelled_Date', $from, $to, $where, $params);
    if ($createdBy !== '') { $where[] = "t.User_id LIKE ?"; $params[] = '%'.$createdBy.'%'; }
    if ($customer  !== '') { $where[] = "COALESCE(t.Cust_name, c.Cust_name) LIKE ?"; $params[] = '%'.$customer.'%'; }

    $sql = "SELECT t.Trans_no, CONVERT(VARCHAR(20), t.Trans_date, 103) AS Trans_date, COALESCE(t.Cust_name, c.Cust_name) AS Cust_name,
                    ISNULL(SUM(td.amount),0) AS bill_amount,
                    CONVERT(VARCHAR(20), t.Cancelled_Date, 103) AS Cancelled_Date, t.Cancelled_By, t.Cancel_Reason
            FROM [Transaction] t
            LEFT JOIN trans_detail td ON td.Trans_no = t.Trans_no
            LEFT JOIN Customer c ON c.Customer_id = t.Customer_id";
    if ($where) $sql .= " WHERE " . implode(' AND ', $where);
    $sql .= " GROUP BY t.Trans_no, t.Trans_date, t.Cust_name, c.Cust_name, t.Cancelled_Date, t.Cancelled_By, t.Cancel_Reason
...
if ($report === 'all_sold_items' || $report === 'item_group_summary') {
    ...
    if ($customer !== '') {
        // Same t.Cust_name-is-always-NULL-on-old-data situation as the
        // Bills-driven reports above -- fall back to the Customer join.
        $joins .= " LEFT JOIN Customer c ON c.Customer_id = t.Customer_id";
        $where[] = "COALESCE(t.Cust_name, c.Cust_name) LIKE ?"; $params[] = '%'.$customer.'%';
    }
    if ($salePerson !== '') {
        $joins .= " LEFT JOIN Employee emp ON emp.Emp_no = t.Sale_Person_id";
        $where[] = "emp.Full_Name LIKE ?"; $params[] = '%'.$salePerson.'%';
    }
...
if ($report === 'price_changed') {
    $where = []; $params = [];
    addDateRange('h.Changed_Date', $from, $to, $where, $params);
    if ($itemCode    !== '') { $where[] = "CAST(s.STOCK_NUMBER AS VARCHAR(20)) LIKE ?"; $params[] = '%'.$itemCode.'%'; }
    if ($itemName    !== '') { $where[] = "(s.ITEM_NAME LIKE ? OR s.BRAND_NAME LIKE ?)"; $params[] = '%'.$itemName.'%'; $params[] = '%'.$itemName.'%'; }
    if ($itemType    !== '') { $where[] = "s.ITEM_TYPE LIKE ?"; $params[] = '%'.$itemType.'%'; }

    $sql = "SELECT h.STOCK_NUMBER, s.BRAND_NAME, s.ITEM_NAME, h.Old_Price, h.New_Price,
                    CONVERT(VARCHAR(20), h.Changed_Date, 103) AS Changed_Date, h.Source
            FROM Item_Price_History h
            JOIN Item_Stock s ON s.STOCK_NUMBER = h.STOCK_NUMBER";
    if ($where) $sql .= " WHERE " . implode(' AND ', $where);
    $sql .= " ORDER BY h.Changed_Date DESC";
    $stmt = runQuery($conn, $sql, $params);
    $rows = [];
    while ($r = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        $rows[] = [

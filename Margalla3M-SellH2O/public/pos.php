...
<div class="win-menubar">
    ...
    <span class="win-menu-item" style="color:#5b3a8a;font-weight:bold;" onclick="window.location='admin_reports.php'" title="Admin only">&#x1F4C8; Profit Reports</span>
...
<!-- ===================== POS VIEW ===================== -->
<div id="view-pos" style="display:flex; flex-direction:column; flex:1; min-height:0; padding:5px; gap:4px;">
    ...
    <div class="win-panel" style="display:flex; align-items:center; gap:8px; padding:4px 8px; flex-wrap:wrap;">
        <div style="display:flex;align-items:center;gap:3px;">
            ...
            <input id="bill-no" type="text" readonly value="—" style="width:65px;font-weight:bold;color:#0a246a;" class="readonly-field" tabindex="-1">
        </div>
        <div style="display:flex;align-items:center;gap:3px;">
...
<!-- ===================== INVENTORY VIEW ===================== -->
<div id="view-inventory" style="display:none;flex-direction:column;flex:1;min-height:0;padding:5px;gap:4px;">
    <div class="win-panel" style="flex:1;min-height:0;display:flex;flex-direction:column;">
        <div class="win-section-label">
...
<!-- ===================== BOOKING VIEW ===================== -->
<div id="view-booking" style="display:none;flex-direction:column;flex:1;min-height:0;padding:5px;gap:4px;">
    <div class="win-panel" style="flex:1;min-height:0;display:flex;flex-direction:column;">
        <div class="win-section-label">
...
function renderProductList(data) {
    ...
    data.forEach(item => {
        ...
        tr.innerHTML = `<td style="font-weight:bold;color:#0a246a;">${item.STOCK_NUMBER}</td><td>${item.BRAND_NAME||'—'}</td><td>${item.SIZE_DESC||item.ITEM_NAME||'—'}</td><td style="text-align:right;">${parseFloat(item.PRICE||0).toFixed(2)}</td>`;
        tr.onclick = () => {
...
function doPrint() {
    const billNo = document.getElementById('bill-no').value;
    if (billNo !== '—') { printSavedInvoice(billNo); return; }
...
function resetForm() {
    ...
    document.getElementById('invno-display').textContent='—';
    document.getElementById('item-search').value='';
    document.querySelectorAll('#product-list-body tr').forEach(r => r.classList.remove('row-selected'));
...
function loadInventory() {
    fetch('api/get_inventory.php').then(r=>r.json()).then(rows => {
        ...
        rows.forEach(row => {
            ...
            tr.innerHTML = `<td style="font-weight:bold;color:#0a246a;">${row.STOCK_NUMBER}</td><td style="font-weight:bold;">${row.BRAND_NAME||'—'}</td><td>${row.ITEM_NAME||'—'}</td><td>${row.ITEM_TYPE||'—'}</td><td>${row.VOLUME_L||'—'}</td><td>${row.SIZE_DESC||'—'}</td><td style="text-align:right;font-weight:bold;">${parseFloat(row.PRICE||0).toFixed(2)}</td><td style="text-align:right;font-weight:bold;color:${qtyColor};">${qty}</td><td>${row.AVAILABLE_STATUS||'—'}</td><td style="font-family:monospace;">${row.BARCODE||'—'}</td>`;
            tbody.appendChild(tr);
        });
    }).catch(() => {
        document.getElementById('inv-body').innerHTML =
            '<tr><td colspan="10" style="text-align:center;color:darkred;padding:10px;">Could not load inventory — check DB connection</td></tr>';
    });
}

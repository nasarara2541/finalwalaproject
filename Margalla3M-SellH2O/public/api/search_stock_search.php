...
// The MargallaProd_Updates migration (SUPPLIER_CODE, SAFETY_LEVEL,
// DISC_STATUS, etc.) was only ever run against MargallaProd (Water
// Distribution) -- MargallaTesting (Med Stock Testing) still has the old
// Item_Stock shape, so this query would throw a raw "Invalid column name"
// SQL error there. Fail with a clear message instead of a broken grid.
if (($_SESSION['active_db_label'] ?? '') !== 'Water Distribution') {
    http_response_code(400);
    echo json_encode(['error' => 'Stock Search needs the Item_Stock schema update that has only been applied to the Water Distribution database — switch database to use this screen.']);
    exit;
}

...
<div style="display:flex;flex-direction:column;flex:1;padding:5px;gap:4px;min-height:0;">
    ...
    <div class="win-panel" style="padding:8px;">
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:6px;">
            <label style="display:flex;align-items:center;gap:4px;font-weight:bold;">
                <input id="chk-from" type="checkbox" checked style="width:auto;height:auto;"> From
            </label>
    ...
    <div style="display:flex;gap:4px;flex:1;min-height:0;">
        ...
        <div class="win-panel" style="flex:1;display:flex;flex-direction:column;min-height:0;padding:8px;overflow:auto;">
            ...
            <div class="field-row"><label class="lbl">Customer</label><input id="f-customer" type="text"></div>
            <div class="field-row"><label class="lbl">Address</label><input id="f-address" type="text"></div>
            <div class="field-row">
                <label class="lbl" style="color:#888;" title="No separate salesperson concept — we only track one User_id per bill. Not wired up yet, noted for follow-up">Sale Person</label>
                <input type="text" disabled title="Not wired up yet — noted for follow-up">
            </div>
            <div class="field-row"><label class="lbl">Ref #</label><input id="f-ref" type="text"></div>
            <div class="field-row">
                <label class="lbl" style="color:#888;" title="No matching column on [Transaction] — not wired up yet, noted for follow-up">Description</label>
                <input type="text" disabled title="Not wired up yet — noted for follow-up">
            </div>
            <div class="field-row">
                <label class="lbl" style="color:#888;" title="No matching column on [Transaction] — not wired up yet, noted for follow-up">Remarks</label>
                <input type="text" disabled title="Not wired up yet — noted for follow-up">
            </div>
            <div class="field-row">
                <label class="lbl" style="color:#888;" title="No audit/last-modified tracking exists anywhere in the schema — not wired up yet, noted for follow-up">Modify By</label>
                <input type="text" disabled title="Not wired up yet — noted for follow-up">
            </div>
            <div class="field-row">
            ...
            <div class="field-row">
                <label class="lbl" style="color:#888;" title="No shift concept exists in the schema — not wired up yet, noted for follow-up">Shift</label>
            ...
            <div class="win-inset" style="margin-top:auto;">
                ...
                <button class="win-btn report-btn" onclick="runReport('day_wise_sale')" title="Sale half is real and working. Return half isn't included — no returns/refunds concept exists anywhere in the app yet; that's coming as its own screen.">Day Wise Sale &amp; Return Summary</button>
                <button class="win-btn report-btn" disabled title="There's no cancel/void mechanism for a saved bill anywhere in the app yet — not wired up yet, noted for follow-up">Cancelled Bills</button>
            </div>
        </div>
        ...
        <div class="win-panel" style="flex:1;display:flex;flex-direction:column;min-height:0;padding:8px;overflow:auto;">
            ...
            <div class="field-row"><label class="lbl">Customer</label><input id="f-item-customer" type="text"></div>
            <div class="field-row">
                <label class="lbl" style="color:#888;" title="No separate salesperson concept — not wired up yet, noted for follow-up">Sale Person</label>
                <input type="text" disabled title="Not wired up yet — noted for follow-up">
            </div>
            ...
            <div class="win-inset" style="margin-top:auto;">
                ...
                <button class="win-btn report-btn" disabled title="PRICE_2/PRICE_3 aren't a fit for this — they're undocumented extra price fields (no comment defining them, likely separate pricing tiers), not a rolling history, and there's no timestamp of when a price changed. Real tracking needs a dedicated price-history table (item, old price, new price, changed date/by). Not wired up yet, noted for follow-up.">Price Changed Item(s)</button>
            </div>
        </div>
...
function runReport(report) {
    ...
    const params = new URLSearchParams({
        ...
        created_by:  (report==='all_sold_items'||report==='item_group_summary') ? document.getElementById('f-item-created-by').value : document.getElementById('f-created-by').value,
        customer:    (report==='all_sold_items'||report==='item_group_summary') ? document.getElementById('f-item-customer').value    : document.getElementById('f-customer').value,
...
// defaulting to the same date in the screenshot).

<?php
// Rest of the Administration dashboard -- Admin + Management, per Zeeshan's
// matrix ("Management: everything EXCEPT Manage Users"). Manage Users itself
// stays strictly Admin-only via require_admin.php elsewhere.
...
<?php $SCREEN_NAME = 'Admin Dashboard'; $SCREEN_ICON = 'gauge'; require __DIR__ . '/includes/navbar.php'; ?>
...
<div class="win-statusbar">
    ...
    <span><?php
        // This note was static text describing a cost basis that only ever
        // matched Med Stock -- Water's real formula is the flat per-container
        // rate (see get_dashboard_summary.php), never "current purchase
        // price". Made accurate for whichever database is actually active.
        $isWaterDb = ($_SESSION['active_db_label'] ?? 'Water Distribution') === 'Water Distribution';
        echo $isWaterDb
            ? 'Cost = a flat Rs./bottle rate by container size (0.5L/1.5L/6L/12L), 19L costs nothing to refill'
            : 'Cost = recorded cost where available, otherwise quantity × current purchase price';
    ?></span>
</div>
